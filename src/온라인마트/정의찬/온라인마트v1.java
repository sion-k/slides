...
                        (o1, o2) -> Integer.compare(o1[0] - discount[cate][comp][o1[2]], o2[0] - discount[cate][comp][o2[2]]));
            }
        }
        isSell = new boolean[50000 + 100];
        idToIdx = new HashMap<Integer, Integer>();
        idxToId = new int[50000 + 100];
        discount = new int[5 + 1][5 + 1][10000 + 100];
        recentVersionNo = new int[5 + 1][5 + 1];
        num = new int[5 + 1][5 + 1];
        idxToCategoryAndCompany = new int[50000 + 100];
        idxToPrice = new int[50000 + 100];
        idxToVersion = new int[50000 + 100];
    }
 
...
        idxToId[idx] = mID;
        isSell[idx] = true;
        pq[mCategory][mCompany].add(new int[] { mPrice, idx, recentVersionNo[mCategory][mCompany] });
        num[mCategory][mCompany]++;
        idxToCategoryAndCompany[idx] = mCategory * 10 + mCompany;
        idxToPrice[idx] = mPrice;
        idxToVersion[idx] = recentVersionNo[mCategory][mCompany];
        idx++;
...
        num[v / 10][v % 10]--;
 
        int version = idxToVersion[targetIdx];
 
        return idxToPrice[targetIdx] - discount[v / 10][v % 10][version];
    }
 
...
        int recentV = ++recentVersionNo[mCategory][mCompany];
 
        // 여기 최적화가능해보임
        for (int i = recentV - 1; i >= 0; i--) {
            discount[mCategory][mCompany][i] += mAmount;
        }
 
        while (!pq[mCategory][mCompany].isEmpty()) {
            int[] current = pq[mCategory][mCompany].peek();
            if (0 < current[0] - discount[mCategory][mCompany][current[2]]) {
                break;
            }
            if(isSell[current[1]])
                num[mCategory][mCompany]--;
...
                        int d = discount[cate][com][c[2]];
                        int id = idxToId[c[1]];
                        candi.add(new int[] { c[0] - d, c[0], c[1], c[2], cate, com, id });
                        if (isSell[c[1]])
                            ansNum++;
                    }
                }
